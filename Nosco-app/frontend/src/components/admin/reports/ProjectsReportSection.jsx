...
import { getProjectsForReport } from '../../../services/adminReportService';
...
const ProjectsReportSection = () => {
  ...
  const handleApplyFilters = async (filters) => {
    setLoading(true);
    try {
      const data = await getProjectsForReport(filters);
      setReportData(data);
    } catch (err) {
      console.error('Error loading projects report:', err);
      setReportData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExportCSV = () => {
    ...
    saveAs(blob, 'projects-report.csv');
  };
